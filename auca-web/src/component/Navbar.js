...
function Toggle({ checked, onChange }) {
  return (
    <div onClick={e => { e.stopPropagation(); onChange(!checked); }}
      style={{ width: '40px', height: '22px', borderRadius: '11px', cursor: 'pointer', flexShrink: 0, background: checked ? '#4d8af0' : '#555', position: 'relative', transition: 'background 0.25s' }}>
      <div style={{ position: 'absolute', top: '1px', left: checked ? '19px' : '1px', width: '20px', height: '20px', borderRadius: '50%', background: '#fff', boxShadow: '0 1px 4px rgba(0,0,0,0.3)', transition: 'left 0.25s' }} />
    </div>
  );
}
...
function Popup({ children, bottom }) {
  return (
    <div style={{ position: 'absolute', bottom: bottom || '70px', left: '12px', right: '12px', background: 'var(--surface)', borderRadius: '16px', border: '1px solid var(--border)', boxShadow: '0 -8px 32px rgba(0,0,0,0.2)', overflow: 'hidden', zIndex: 300, animation: 'popupIn 0.18s ease' }}>
      {children}
    </div>
  );
}
...
function PopupRow({ iconBg, iconColor, icon, label, sublabel, right, onClick, danger, border = true }) {
  return (
    <div onClick={onClick}
      style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '13px 16px', cursor: 'pointer', borderBottom: border ? '1px solid var(--border)' : 'none', transition: 'background 0.15s' }}
      onMouseEnter={e => e.currentTarget.style.background = danger ? '#fff0f0' : 'var(--surface-2)'}
      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
    >
      <div style={{ width: '32px', height: '32px', borderRadius: '8px', flexShrink: 0, background: iconBg || 'var(--primary-pale)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: iconColor || 'var(--primary)' }}>
        {icon}
      </div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: '13px', fontWeight: 700, color: danger ? '#e53935' : 'var(--text-primary)' }}>{label}</div>
...
export default function Navbar({ activePage, onNavigate, theme, onThemeChange, onLogout }) {
  ...
  // Refresh user info when localStorage changes (e.g. after login)
  useEffect(() => {
